...
class CausalFlowNet(nn.Module):
    ...
    def anm_direction_test(self, data, i, j):
        """
        Perform Additive Noise Model (ANM) directional test between two variables.
        Thực hiện kiểm định chiều nhân quả theo Mô hình Nhiễu Cộng (ANM) cho 2 biến.
        Hypothesis: If i -> j is correct, then Cause(X_i) is statistically independent of Residual e_j = X_j - f(X_i).
        Returns the preferred direction: The one with smaller HSIC (more independent).
        """
        self.eval() # Evaluation mode / Chế độ đánh giá
        ...
        with torch.no_grad():
            # Test 1: i -> j (Predict j from i) / Kiểm tra chiều i -> j
            x_in_j = torch.zeros((B, self.n_vars), device=device)
            x_in_j[:, i] = X[:, i] # Giữ lại mỗi biến i
            pred_j = self.shared_phi(x_in_j) # Dự báo j
            res_j = X[:, j:j+1] - pred_j # Phần dư của j

            # Test 2: j -> i (Predict i from j) / Kiểm tra chiều j -> i
            x_in_i = torch.zeros((B, self.n_vars), device=device)
            x_in_i[:, j] = X[:, j] # Giữ lại mỗi biến j
            pred_i = self.shared_phi(x_in_i) # Dự báo i
            res_i = X[:, i:i+1] - pred_i # Phần dư của i

            # Simple HSIC function with RBF kernel (Hilbert-Schmidt Independence Criterion)
            # Hàm tính toán điểm độc lập bằng Kernel Gauss RBF
            def hsic_rbf(var_src, res_target, sigma=1.0):
                dists_x = torch.cdist(var_src, var_src)
                K_x = torch.exp(-dists_x**2 / (2 * sigma**2))
                dists_y = torch.cdist(res_target, res_target)
                ...
                K_xc = torch.mm(K_x, H)
                K_yc = torch.mm(K_y, H)
                return torch.sum(K_xc * K_yc).item() / ((B - 1)**2)

            # Compute independence: Cause vs Target's Residual
            hsic_i_to_j = hsic_rbf(X[:, i:i+1], res_j)
            hsic_j_to_i = hsic_rbf(X[:, j:j+1], res_i)

        # Smaller HSIC means stronger independence -> That direction fits Additive Noise Model better
        # HSIC càng nhỏ đồng nghĩa với Độc lập thống kê càng cao -> Đúng quy luật ANM
        if hsic_i_to_j < hsic_j_to_i:
            preferred_dir = (i, j)
        else:
            preferred_dir = (j, i)
            
        return {
            "preferred_edge": preferred_dir,
            "hsic_i_to_j": hsic_i_to_j,
            "hsic_j_to_i": hsic_j_to_i,
            "confidence_ratio": hsic_j_to_i / (hsic_i_to_j + 1e-9) if preferred_dir == (i, j) else hsic_i_to_j / (hsic_j_to_i + 1e-9)
        }
