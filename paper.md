...
Trong đó:
* $\mathcal{L}_{\text{NLL}} = -\sum_{j=1}^d \log p(\varepsilon_j)$ là hàm âm log-likelihood của dữ liệu quan sát dưới luồng chuẩn hóa rational-quadratic splines đã học.
* $\mathcal{L}_{\text{HSIC}} = \sum_{j=1}^d \log\left(\widehat{\text{HSIC}}(j) + \epsilon_{\text{stab}}\right)$ là tổng hàm phạt độc lập trên tất cả $d$ nút, thúc đẩy trực tiếp tính độc lập giữa cha và phần dư (với $\epsilon_{\text{stab}}$ là hằng số ổn định số học).
* $\|\mathbf{W}\|_1 = \sum_{i \neq j} |W_{ij}|$ là chuẩn $L_1$ của các trọng số ngoài đường chéo, có tác dụng áp đặt tính thưa (sparsity) cho cấu trúc đồ thị.
* $h(\mathbf{W}) = 0$ là ràng buộc phi chu trình trơn (NOTEARS), trong đó $\circ$ là phép nhân Hadamard (element-wise product) và $e^{\mathbf{A}}$ là hàm mũ ma trận.
