...
Trong đó:
* **Hàm cơ chế nhân quả phi tuyến** ($f\sb{j}$):
  $$f_j: \mathbb{R}^{|\mathbf{PA}_j|} \rightarrow \mathbb{R}$$
  biểu diễn cơ chế tác động phi tuyến liên tục.
* **Tập hợp các nút cha trực tiếp** ($\mathbf{PA}\sb{j}^{\mathcal{G}}$):
  $$\mathbf{PA}_j^{\mathcal{G}} \subseteq \mathbf{V} \setminus \{X_j\}$$
  đại diện cho các biến tác động trực tiếp lên $X_j$ trong đồ thị $\mathcal{G}$.
* **Biến nhiễu cấu trúc độc lập** ($\varepsilon\sb{j}$):
  Đại diện cho sai số ngẫu nhiên của mô hình cấu trúc.
...
Thay vì áp đặt giả định nhiễu Gauss thông thường, CausalFlowNet học mật độ chính xác của từng phần dư cấu trúc bằng cách sử dụng **Luồng Spline Nơ-ron (Neural Spline Flow - NSF)** [3]. Đối với mỗi biến $X\sb{j}$, phần dư $\varepsilon\sb{j} = X\sb{j} - f\sb{j}(\mathbf{PA}\sb{j}^{\mathcal{G}})$ được ánh xạ qua một hàm khả nghịch $g\sb{\boldsymbol{\theta}}: \mathbb{R} \rightarrow \mathbb{R}$ để chuyển sang một biến ẩn $z\sb{j}$. Áp dụng định lý biến đổi biến số (change-of-variables), ta thu được hàm log-likelihood chính xác và khả vi:
...
Luồng $g\sb{\boldsymbol{\theta}}$ được hiện thực hóa bằng sự kết hợp của các tầng liên kết **Rational-Quadratic Spline** (RQS). Đây là các phép biến đổi đơn điệu, khả nghịch giải tích, có các tham số được dự báo bởi một mạng nơ-ron nhỏ, cho phép mô hình hóa các phân phối phần dư phức tạp, phi Gauss một cách cực kỳ linh hoạt.
...
Trong đó, các trọng số hỗn hợp $\pi\sb{c}$, kỳ vọng $\mu\sb{c}$, và phương sai $\sigma\sb{c}^2$ là các tham số được tối ưu hóa đồng thời đầu-cuối thông qua thuật toán lan truyền ngược gradient.
...
Trong đó:
* **Ma trận đặc trưng RFF của biến cha** ($\tilde{\Phi}\sb{X}^{(j)} \in \mathbb{R}^{n \times m}$):
  Đại diện cho các đặc trưng Fourier ngẫu nhiên được trích xuất từ nhóm các nút cha của nút $j$.
* **Ma trận đặc trưng RFF của phần dư** ($\tilde{\Phi}\sb{\varepsilon}^{(j)} \in \mathbb{R}^{n \times m}$):
  Đại diện cho đặc trưng RFF được trích xuất từ phần dư cấu trúc của nút $j$.
* **Chuẩn Frobenius bình phương** ($\lVert \cdot \rVert\sb{F}^2$):
  Ký hiệu cho độ lớn toán học của ma trận sau khi nhân chéo đặc trưng.
...
| Giai đoạn | Số lượng Epoch | Tham số phạt $L\sb{1}$ ($\lambda\sb{L\sb{1}}$) | Mục tiêu chính |
| :--- | :---: | :---: | :--- |
