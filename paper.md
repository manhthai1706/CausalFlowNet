...
Trong đó:
* $f_j: \mathbb{R}^{\lvert\mathbf{PA}_j\rvert} \rightarrow \mathbb{R}$ là một hàm phi tuyến liên tục bất kỳ biểu diễn cơ chế nhân quả.
...
$$\widehat{\text{HSIC}}(j) = \frac{1}{(n-1)^2} \left\lVert \left(\tilde{\Phi}_X^{(j)}\right)^\top \tilde{\Phi}_\varepsilon^{(j)} \right\rVert_F^2$$
...
$$\min_{\mathbf{W}, \boldsymbol{\theta}} \quad \mathcal{L}_{\text{main}}(\mathbf{W}, \boldsymbol{\theta}) = \mathcal{L}_{\text{NLL}}(\mathbf{W}, \boldsymbol{\theta}) + \lambda_{\text{HSIC}} \mathcal{L}_{\text{HSIC}}(\mathbf{W}, \boldsymbol{\theta}) + \lambda_{L_1} \lVert\mathbf{W}\rVert_1$$
...
Sau khi quá trình tối ưu hóa liên tục hội tụ, ma trận trọng số liên tục $\mathbf{W}$ được nhị phân hóa thành ma trận kề có hướng $\widehat{\mathbf{A}}$ bằng phương pháp **ngưỡng cắt tỉa thích ứng dựa trên phân phối dữ liệu**: một cạnh $i \rightarrow j$ được giữ lại nếu và chỉ nếu trọng số $\lvert W_{ij}\rvert$ vượt quá giá trị trung bình của toàn bộ các trọng số ngoài đường chéo cộng thêm $\kappa$ lần độ lệch chuẩn:

$$\tau = \text{mean}\left(\lVert\mathbf{W}_{\text{off-diag}}\rVert\right) + \kappa \cdot \text{std}\left(\lVert\mathbf{W}_{\text{off-diag}}\rVert\right)$$
