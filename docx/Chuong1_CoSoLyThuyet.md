...
2. **Cơ chế Cổng (Gating):** Ánh xạ đặc trưng lên không gian 2D, sau đó tách thành hai thành phần — features và gate — điều tiết lẫn nhau theo công thức **$h = \sigma_{\text{act}}(\text{features}) \odot \sigma(\text{gate})$**. Trong đó $\sigma_{\text{act}}$ là hàm kích hoạt đặc trưng (như SiLU), $\sigma$ là hàm Sigmoid ép giá trị cổng về khoảng $(0,1)$, và $\odot$ là phép nhân từng phần tử. Cơ chế này cho phép mô hình chủ động "bật/tắt" các tín hiệu đặc trưng tùy theo ngữ cảnh dữ liệu.
...
**Điều kiện:** $\quad h(W) = \text{Tr}(e^{W \odot W}) - d = 0$

Trong đó NLL là hàm mất mát âm log-likelihood từ Normalizing Flow, $\mathcal{L}_{\text{HSIC}}$ là hạng phạt HSIC, $\|W\|_1$ là chuẩn L1 khuyến khích ma trận kề thưa (sparse), $\lambda_{\text{HSIC}}$ và $\lambda_{L1}$ là các hệ số điều chuẩn, và $h(W) = 0$ là ràng buộc acyclicity.
