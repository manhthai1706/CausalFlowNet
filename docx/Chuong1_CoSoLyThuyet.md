...
Tính chất quan trọng nhất của DAG là tính acyclicity (không chu trình). Để đảm bảo đồ thị học được từ dữ liệu là một DAG hợp lệ, CausalFlowNet sử dụng hàm ràng buộc sau (được triển khai trong `Optimization.py`):
...
Trong CausalFlowNet, hàm cấu trúc f_i được xấp xỉ bởi một mạng Perceptron Nhiều Lớp (MLP) dùng chung (`shared_phi`) cho tất cả các nút. Phần dư sau khi trừ giá trị dự báo chính là phần nhiễu ε_i, và tính độc lập của ε_i với các biến đầu vào là điều kiện cần để xác nhận cấu trúc nhân quả tìm được.
...
Để tăng cường khả năng biểu diễn và giúp mô hình học được các cơ chế nhân quả phức tạp, CausalFlowNet triển khai kiến trúc **Gated Residual Block** (trong `MLP.py`). Cơ chế hoạt động của khối này gồm ba bước:
...
CausalFlowNet sử dụng **Neural Spline Flow** (NSF) với lớp ghép **Rational-Quadratic Splines** (RQS) được triển khai trong `Flow.py`. Thay vì các phép biến đổi affine đơn giản, RQS chia miền đầu vào thành nhiều thùng (bins) và xấp xỉ từng đoạn bằng một hàm hữu tỷ bậc hai, cho phép biểu diễn phân phối phức tạp và đa dạng một cách linh hoạt.
...
Thay vì dùng Gauss đơn giản làm phân phối ưu tiên, CausalFlowNet sử dụng **Gaussian Mixture Model (GMM)** có thể học được (`GaussianMixturePrior`). GMM là tổ hợp tuyến tính của K thành phần Gaussian:
...
Tính toán HSIC chính xác có độ phức tạp O(N²), không khả thi với tập dữ liệu lớn. CausalFlowNet giải quyết vấn đề này bằng lớp **ParallelFastHSIC** (`HSIC.py`), sử dụng **Random Fourier Features** (RFF) để xấp xỉ hàm nhân Gauss:
...
Với γ là hệ số tăng trưởng và ρ_max là giá trị phạt tối đa (được giới hạn để đảm bảo ổn định số học). Quá trình này được triển khai trong lớp `AugmentedLagrangian` (`Optimization.py`), đảm bảo mô hình hội tụ về một DAG hợp lệ.
...
Cả ba chỉ số trên đều được triển khai trong `Evaluation.py` và được báo cáo trong mọi thực nghiệm.
