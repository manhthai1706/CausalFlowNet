...
2. **Cơ chế Cổng (Gating):** Ánh xạ đặc trưng lên không gian 2D, sau đó tách thành hai thành phần — features và gate — điều tiết lẫn nhau theo công thức **$h = \sigma_{\text{act}}(\text{features}) \odot \sigma(\text{gate})$**. Cơ chế này cho phép mô hình chủ động "bật/tắt" các tín hiệu đặc trưng tùy theo ngữ cảnh dữ liệu.
...
Trong đó NLL là hàm mất mát âm log-likelihood từ Normalizing Flow, $\mathcal{L}_{\text{HSIC}}$ là hạng phạt HSIC, $\|W\|_1$ là chuẩn L1 khuyến khích ma trận kề thưa (sparse), và $h(W) = 0$ là ràng buộc acyclicity.
