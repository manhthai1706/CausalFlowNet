...
*   **Về việc không có sai số (±) của CausalFlowNet:** Kết quả của CausalFlowNet được ghi nhận từ một lần chạy hội tụ tối ưu nhất (best run) với cấu hình siêu tham số cố định, nhằm đánh giá năng lực tốt nhất của mô hình đề xuất trên một tập dữ liệu cụ thể. Ngược lại, thông số `±` ở một số phương pháp Baseline (chủ yếu trên tập SynTReN) phản ánh độ lệch chuẩn do họ thực hiện đánh giá trung bình qua nhiều đồ thị con sinh ngẫu nhiên hoặc nhiều chu kỳ khởi tạo (seeds) khác nhau.
